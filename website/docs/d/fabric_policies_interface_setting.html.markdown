...
* `cdp_admin_state` - (Read-Only) The administrative state of CDP (Cisco Discovery Protocol).
* `lldp_receive_state` - (Read-Only) The receive state of LLDP (Link Layer Discovery Protocol).
* `lldp_transmit_state` - (Read-Only) The transmit state of LLDP.
* `llfc_receive_state` - (Read-Only) The receive state of LLFC (Link Level Flow Control).
* `llfc_transmit_state` - (Read-Only) The transmit state of LLFC.
* `pfc_admin_state` - (Read-Only) The administrative state of PFC (Priority Flow Control).
...
* `stp_bpdu_filter` - (Read-Only) Enable or disable BPDU (Bridge Protocol Data Unit) filter for STP (Spanning Tree Protocol).
* `stp_bpdu_guard` - (Read-Only) Enable or disable BPDU guard for STP.
...
* `link_level_fec` - (Read-Only) The FEC (Forwarding Error Correction) mode.
* `mcp_admin_state` - (Read-Only) The administrative state of MCP (Mis-Cabling Protocol).
* `mcp_strict_mode` - (Read-Only) Enable or disable strict mode of MCP.
* `mcp_initial_delay_time` - (Read-Only) The initial delay time of MCP in seconds.
...
* `port_channel_mode` - (Read-Only) The port channel mode of the Interface Setting.
* `controls` - (Read-Only) A list of port channel controls.
* `port_channel_min_links` - (Read-Only) The minimum number of active links in the port channel.
* `port_channel_max_links` - (Read-Only) The maximum number of links in the port channel.
* `load_balance_hashing` - (Read-Only) The load balancing hashing algorithm of the Interface Setting.
