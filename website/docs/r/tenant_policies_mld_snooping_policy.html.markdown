...
`admin_state` - (Optional) The administrative state of the MLD Snooping Policy. Valid values: `enabled`, `disabled`.
`fast_leave_control` - (Optional) Enable or disable fast leave processing. When enabled, the switch immediately removes a multicast group when it receives an MLD Done message.
`querier_control` - (Optional) Enable or disable querier control. When enabled, the switch can act as an MLD querier.
`querier_version` - (Optional) The MLD querier version. Default: v2 when unset during creation. Valid values: `v1`, `v2`.
`query_interval` - (Optional) The interval in seconds between MLD general query messages. Valid range: 1-18000 seconds.
`query_response_interval` - (Optional) The maximum time in seconds that hosts can wait before responding to an MLD query. Valid range: 1-25 seconds.
`last_member_query_interval` - (Optional) The interval in seconds between MLD group-specific queries sent in response to an MLD Done message. Valid range: 1-25 seconds.
`start_query_interval` - (Optional) The interval in seconds between MLD queries sent at startup. Valid range: 1-18000 seconds.
`start_query_count` - (Optional) The number of MLD queries sent at startup. Valid range: 1-10.
