...
// errors -- including an apply that fails on a data source read. The error
// flow tests in TestAccMSOSchemaSiteDatasource intentionally trigger such
// failures, so the prereq tenant/schema/site association created during the
// failing apply are orphaned on MSO (present on the server, absent from
// Terraform state). Without explicit cleanup, the next step (or test run)
// fails on "Tenant/Schema: '...' already exists".
//
// Cleanup order: schema first (so the schema_site association inside it goes
// with it), then tenant. The underlying delete helpers live in
// test_constants.go so other acceptance tests that intentionally trigger
// apply-time errors can reuse them.
