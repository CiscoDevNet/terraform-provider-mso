...
func resourceMSOTemplate() *schema.Resource {
	return &schema.Resource{
		...
		// Can we do validation here due to unknown values?
		CustomizeDiff: func(diff *schema.ResourceDiff, v interface{}) error {
			...
			// 		templateType: diff.Get("template_type").(string),
			// 		tenantName:   diff.Get("tenant_name").(string),
			// 		sites:        sites,
			// 	}

			// 	// This check is also done in the validateConfig function to prevent create and update scenarios with unknown values for sites
			...
			// 	// This check is also done in the validateConfig function to prevent create and update scenarios with unknown values for sites
			// 	duplicates := duplicatesInList(sites)
			// 	if len(duplicates) > 0 {
			...
			// 		}
			// 		return errors.Join(duplicatesErrors...)
			// 	}
			// 	// old = [a,b], new = [b,a]
...
// Should we validate or let API handle it if possible?
