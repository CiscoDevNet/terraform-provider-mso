...
func TestAccMSODHCPRelayPolicyProvider_Basic(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: MSODHCPRelayPolicyProviderWithExtEpg(tenantNames[0], schemaName, name, addr),
				Check: resource.ComposeTestCheckFunc(
			...
			{
				Config: MSODHCPRelayPolicyProviderWithExtEpg(tenantNames[0], schemaName, nameOther, addr),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckMSODHCPRelayPolicyProviderExists(resourceName, &prov2),
					resource.TestCheckResourceAttr(resourceName, "dhcp_relay_policy_name", nameOther),
					resource.TestCheckResourceAttr(resourceName, "dhcp_server_address", addr),
					resource.TestCheckResourceAttrSet(resourceName, "external_epg_ref"),
					testAccCheckMSODHCPRelayPolicyProviderIdNotEqual(&prov1, &prov2),
				),
			...
			{
				Config: MSODHCPRelayPolicyProviderWithExtEpg(tenantNames[0], schemaName, name, addrOther),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckMSODHCPRelayPolicyProviderExists(resourceName, &prov2),
					resource.TestCheckResourceAttr(resourceName, "dhcp_relay_policy_name", name),
					resource.TestCheckResourceAttr(resourceName, "dhcp_server_address", addrOther),
					resource.TestCheckResourceAttrSet(resourceName, "external_epg_ref"),
					testAccCheckMSODHCPRelayPolicyProviderIdNotEqual(&prov1, &prov2),
				),
			},
...
func TestAccMSODHCPRelayPolicyProvider_Negative(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config:      MSODHCPRelayPolicyProviderWithEpg(tenantNames[0], name, randomValue, epg),
				ExpectError: regexp.MustCompile(`expected (.)+ to contain a valid IPv4 address`),
			},
			{
				Config:      MSODHCPRelayPolicyProviderWithEpg(tenantNames[0], name, addr, randomValue),
				ExpectError: regexp.MustCompile(`invalid`),
			},
			{
				Config:      MSODHCPRelayPolicyProviderWithInvalidExtEpg(tenantNames[0], name, addr, randomValue),
				ExpectError: regexp.MustCompile(`invalid`),
			},
			{
				Config:      MSODHCPRelayPolicyProviderAttr(tenantNames[0], name, addr, epg, randomParameter, randomValue),
				ExpectError: regexp.MustCompile(`An argument named(.)+is not expected here.`),
			},
			{
				Config: MSODHCPRelayPolicyProviderWithEpg(tenantNames[0], nameOther, addr, epg),
			},
		},
	})
}
...
func MSODHCPRelayPolicyProviderWithExtEpg(tenant, schema, name, addr string) string {
	...
	`, schema, schema, name, name, name, name, addr)
	return resource
}

func MSODHCPRelayPolicyProviderWithInvalidExtEpg(tenant, name, addr, extepg string) string {
	resource := CreateDHCPRelayPolicy(tenant, name)
	resource += fmt.Sprintf(`
	resource "mso_dhcp_relay_policy_provider" "test" {
		dhcp_relay_policy_name = mso_dhcp_relay_policy.test.name
...
func MSODHCPRelayPolicyProviderAttr(tenant, name, addr, epg, key, value string) string {
	resource := CreateDHCPRelayPolicy(tenant, name)
	resource += fmt.Sprintf(`
	resource "mso_dhcp_relay_policy_provider" "test" {
