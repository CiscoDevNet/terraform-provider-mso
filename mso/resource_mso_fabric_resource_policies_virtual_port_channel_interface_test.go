...
func TestAccMSOVirtualPortChannelInterfaceResource(t *testing.T) {
	resource.Test(t, resource.TestCase{
			...
			},
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr(
						"mso_fabric_resource_policies_virtual_port_channel_interface.vpc_if",
						"description",
						"Terraform test VPC Interface (removed descriptions)",
					),
					resource.TestCheckResourceAttr(
						"mso_fabric_resource_policies_virtual_port_channel_interface.vpc_if",
						"node_1_interfaces.#",
		...
		CheckDestroy: testCheckResourceDestroyPolicyWithPathAttributesAndArguments("mso_fabric_resource_policies_virtual_port_channel_interface", "fabricResourceTemplate", "template", "virtualPortChannels"),
	})
}
...
resource "mso_fabric_resource_policies_virtual_port_channel_interface" "vpc_if" {
  ...
  interface_descriptions {
      node         = "101"
      interface    = "1/1"
      description  = "Terraform test interface description 101"
  }

  interface_descriptions {
      node         = "102"
      interface    = "1/3"
      description  = "Terraform test interface description 102"
  }
}
`, testAccMSOTemplateResourceFabricResourceConfig())
}
