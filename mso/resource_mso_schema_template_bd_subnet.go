...
func resourceMSOTemplateBDSubnetUpdate(d *schema.ResourceData, m interface{}) error {
	...
	path := fmt.Sprintf("/templates/%s/bds/%s/subnets/%s", templateName, bdName, d.Id())
	bdSubnetStruct := models.NewTemplateBDSubnet("replace", path, IP, Desc, Scope, Shared, NoDefaultGateway, Querier)
...
func resourceMSOTemplateBDSubnetDelete(d *schema.ResourceData, m interface{}) error {
	...
	path := fmt.Sprintf("/templates/%s/bds/%s/subnets/%s", templateName, bdName, d.Id())
	bdSubnetStruct := models.NewTemplateBDSubnet("remove", path, IP, Desc, Scope, Shared, NoDefaultGateway, Querier)
	_, err := msoClient.PatchbyID(fmt.Sprintf("api/v1/schemas/%s", schemaID), bdSubnetStruct)
